...
from util.get_point import find_match_points_surf
...
if __name__ == '__main__':
    ...
    # todo 提取特征点
    (src_point, dst_point) = find_match_points_surf(src=src, dst=dst)

    # todo 利用ransac来对特征点进行筛选
    final_num, final_src_point, final_dst_point = ransac(src_point=src_point, dst_point=dst_point, n=300, threshold=15,
                                                         n_iter=20000)
    # todo 显示处理后的特征点位置
    ...
    for i in range(grid_center_point.shape[0]):
        for j in range(grid_center_point.shape[1]):
            weight, w = calculate_weight(grid_center_point[i, j, :], final_dst_point, 0.1, 11)
